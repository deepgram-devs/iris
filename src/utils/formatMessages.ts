...
/**
 * Takes an array of Discord messages and formats them into the
 * typical LLM "role" and "content" format. Also appends the frontmatter
 * to each message.
 * @param messages - The messages to format.
 * @param channelName - The name of the channel the messages were sent in.
...
const formatDiscordMessages = (
  ...
  const formattedMessages = messages.map((message) => {
    const role = message.author.id === irisId
      ? "assistant"
      : "user";
    return {
      content: appendDiscordFrontmatter(message, channelName, username, irisId),
      role:    role,
    };
  });
  return formattedMessages;
...
export { formatSlackMessages, formatDiscordMessages };
